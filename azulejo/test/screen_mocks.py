...
class MultipleTestScreenMock(ScreenMockBase):
    ...
    In a simple side by side configuration where the active window is on the
    2nd monitor

    TODO additional types of multiple test config:
        - two monitors arranged top and bottom
        - >2 monitors
        - monitors where one is larger than the other (and moving a too large
          window to smaller one)
